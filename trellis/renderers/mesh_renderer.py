...
class MeshRenderer:
    ...
    def render(
        ...
        if "color" in return_types:
            # bg_color is (R,G,B) in 0-255 or 0-1. Assume 0-255 and normalize
            color_bg = torch.tensor(bg_color, dtype=torch.float32, device=self.device)
            if color_bg.max() > 1.0:
                color_bg = color_bg / 255.0
            color_bg = color_bg.view(1, 3, 1, 1)  # shape for broadcasting
        ...
        for type in return_types:
            ...
            if type == "mask" :
                ...
                img = dr.interpolate(mesh.vertex_attrs[:, :3].contiguous(), rast, faces_int)[0]
                img = dr.antialias(img, rast, vertices_clip, faces_int)
                # set background to bg_color (rgb in 0-1)
                if color_bg is not None:
                    # rast[..., -1:] > 0 gives mask (B, H, W, 1)
                    mask = (rast[..., -1:] > 0).float().permute(0, 3, 1, 2)  # (B, 1, H, W)
                    img = img.permute(0, 3, 1, 2)  # (B, C, H, W)
                    img = img * mask + (1 - mask) * color_bg
                    img = img.permute(0, 2, 3, 1)  # back to (B, H, W, C) for further handling

            if ssaa > 1:
                img = img.permute(0, 3, 1, 2)
                img = F.interpolate(img, (resolution, resolution), mode='bilinear', align_corners=False, antialias=True)
                img = img.squeeze()
            else:
